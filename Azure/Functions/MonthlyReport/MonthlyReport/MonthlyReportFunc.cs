...
namespace MonthlyReport
{
    public class MonthlyReportFunc
    {
        ...
        private string CreateBody(UserProfileDto user, MonthSummaryDto currentMonthSummary, MonthSummaryDto prevMonthSummary, int currentYear, int currentMonth, int prevYear, int prevMonth)
        {
            ...
            htmlContent += $"<div style=\"margin-bottom: 1.5rem;\"><span style=\"font-family: Verdana, sans-serif; font-size: 1rem;\">{string.Format(Resources.Resource.ResourceManager.GetString("DATE", CultureInfo.GetCultureInfo(user.Language)), $"<div style=\"font-weight: bold; display: contents;\">{new DateOnly(currentYear, currentMonth, 1).ToString("MMMM yyyy", CultureInfo.GetCultureInfo(user.Language))}</div>")}</span></div>";
            ...
            htmlContent += $"<li><div><span style=\"font-family: Verdana, sans-serif; font-size: 0.7rem;\">{string.Format(strComparisonIncome, user.Currency, incomeComp, new DateOnly(prevYear, prevMonth, 1).ToString("MMMM yyyy", CultureInfo.GetCultureInfo(user.Language)))}</span></div></li>";
            htmlContent += "</ul>";
            ...
            htmlContent += "<ul>";
            htmlContent += $"<li><div><span style=\"font-family: Verdana, sans-serif; font-size: 0.7rem;\">{string.Format(strComparisonOutcome, user.Currency, outcomeComp, new DateOnly(prevYear, prevMonth, 1).ToString("MMMM yyyy", CultureInfo.GetCultureInfo(user.Language)))}</span></div></li>";
            htmlContent += "</ul>";
            htmlContent += "</ul>";
